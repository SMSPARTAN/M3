...
{                                                                         // no arquivo a formatação é nome presença, presença sendo representa por 1 = sim e 0 = não
    ...
    {                        // verifica se a função foi chamada (informado no argumento retType) como 1 ou não, se for chamada como 1,
        return nome.c_str(); // retorna a parte do nome da string se não, retorna a parte númerica
    }
    return num.c_str();
}
...
int main()
{
    std::printf("Nome: %s\nNumber: %s", separarNomeDeConfirmado(str, 1), separarNomeDeConfirmado(str, 0));
