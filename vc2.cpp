...
int main()
{
    std::setlocale(LC_ALL, ("Portuguese")); // define o locale para pt_BR para imprimir caracteres do alfabeto Português
    ...
    while(!adicionar && !remover && !pesquisar && !imprimir) // enquanto o usuário não escolher uma opção válida, repete o bloco em loop.
    {
        ...
        {
        case 1: // dependendo da escolha do usuário executa limparCin() e define booleana relaciona a opção escolhida como verdadeira.
            limparCin();
            adicionar = true;
            break;
        case 2:
        ...
        case 4:
            ...
            break;
        default:
            limparCin(); // caso o input seja inválido - Ex:. usuário digita uma letra ou escolhe uma opção maior que 3 e menor que 1 -- retorna uma mensagem de erro e limpa o cin
            std::cout << "\n\n -- Escolha Inválida -- \n\n";
            break;
        }
    }
    ...
    while(adicionar) // repete o bloco em loop se a opção 'adicionar' for escolhida
    {
        ...
        std::getline(std::cin, convidado); // le a linha inteira digitada pelo usuário e armazena o input em 'convidado'
    ...
    while(remover) // repete o loop caso o usuário tenha escolhido a opção de remover um convidado da lista
    {
        ...
        convidados.erase(it);                                       // se encontra dentor do vetor e usa a função 'vector.erase()' para apagar o item na posição
    }                                                               // na qual o iterador aponta

    while(pesquisar) // repte o loop caso o usuário tenha escolhido a opção de pesquisar por um convidado
    {
        ...
        if(!achado) // caso o nome do convidado não esteja na lista, a varíavel achado permanece falsa e, após o fim do loop for, retorna a mensagem "Convidado não está na lista!"
        {
    ...
    if(imprimir)
    {
        for(std::string& nome : convidados)
        {
            std::cout << nome << '\n';
        }
    }
